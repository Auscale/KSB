    ...
    if(isset($_POST['sub_type']) && isset($_POST['sub_title'])){
      if($_POST['sub_type'] == 'image'){
        if($_POST['online_link']){
          echo($_POST['online_link']);
          //requires allow_url_fopen set to true
          $file_name = uniqid();
          $url = $_POST['online_link'];
          $got_image = file_get_contents($url);
          $extention = getimagesize($got_image);
          $extention = image_type_to_extension($extention);
          $img = "uploads/" . $file_name . $extention;
          file_put_contents($img, $got_image);
          echo("That should've worked D:");
        } else {
          $target_dir = "uploads/";
          ...
          if ($_FILES["file_upload"]["size"] > $max_size) {
            echo "Sorry, your file is too large. Current max size is 20MB. If you're not being silly, maybe I could increase it.";
            $uploadOk = 0;
          }
          ...
          if ($uploadOk == 1) {
            if (move_uploaded_file($_FILES["file_upload"]["tmp_name"], $full_name)) {
              ...
              if($_FILES['file_upload']['error'] == 0 && $uploadOk == 1){
                //create sql entry
                $sub_title = mysqli_real_escape_string($con,$_POST['sub_title']);
                ...
                //generate_image_thumbnail($full_name, "uploads/thumbs/" . $file_name . ".jpg");
              }
            } else {
              echo "Something went wrong! Try uploading it again, see if that works.";
            }
          }
        }
